        ...
        foreach (C_AdditionalModifiers addModifier in extras)
        {
            switch (addModifier.targetType)
            {
                ...
                case E_ModifierTargetType.timeline:
                    // Removed legacy timeline integration. Serialized cards may still contain
                    // this target type, so keep it as a no-op for backward compatibility.
                    break;
                case E_ModifierTargetType.gamelog:
                    // Removed legacy game log integration. Serialized cards may still contain
                    // this target type, so keep it as a no-op for backward compatibility.
                    break;
